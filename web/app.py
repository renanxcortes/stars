...
image_filename_webstars = 'WEBSTARS_logo.png'
encoded_image_webstars = base64.b64encode(open(image_filename_webstars, 'rb').read())

image_filename_plotly = 'plotly_logo.png'
encoded_image_plotly = base64.b64encode(open(image_filename_plotly, 'rb').read())

image_filename_nsf = 'NSF_logo.png'
encoded_image_nsf = base64.b64encode(open(image_filename_nsf, 'rb').read())

image_filename_cgs = 'CGS_logo.png'
encoded_image_cgs = base64.b64encode(open(image_filename_cgs, 'rb').read())
...
app.layout = html.Div(
        ...
        html.Div([
                ...
                html.H2(children='Web STARS', style = {'textAlign': 'center'}),
                #html.P('Web STARS'),
                html.Div([
                
                html.A([
                html.Img(src='data:image/png;base64,{}'.format(encoded_image_nsf.decode()), 
                 style={'width': '75px',
                'margin-left': 775})], href='https://www.nsf.gov/'),
    
                html.A([
                html.Img(src='data:image/png;base64,{}'.format(encoded_image_cgs.decode()), 
                style={'width': '75px'})], href='https://spatial.ucr.edu/')], className="row", style={'display': 'inline-block', 'float': 'center'}),
                    
                html.H5(children='A web-based version of Space-Time Analysis of Regional Systems',
                ...
                html.H3(children='This is a beta-version of some STARS/giddy functions using Dash from plotly.', 
                style={'textAlign': 'center',
                   'margin-top': '60', 
                   'font-size': '110%'}),
    
                html.H2(children='- Exploratory Spatial Data Analysis (ESDA)', 
                style={'textAlign': 'center','margin-top': '10', 'font-size': '100%'}),
                        
                html.H2(children='- Rank Paths', 
                style={'textAlign': 'center','margin-top': '10', 'font-size': '100%'}),
                        
                html.H2(children='- Geospatial Distribution Dynamics (Giddy)', 
                style={'textAlign': 'center','margin-top': '10', 'font-size': '100%'}),
                
                html.Img(src='data:image/png;base64,{}'.format(encoded_image_webstars.decode()), 
                 style={'width': '150px',
                'margin-left': 775} ),
                ...
                html.P('This current prototype was supported by the National Science Foundation.'),
    
                dcc.Upload(
                    id='upload-csv',
                    children=html.Div([
                        'Drag and Drop or ',
                        html.A('Select'), ' comma separated value (.csv) (not implemented)'
                    ]),
                    style={
                        #'width': '25%',
                        'height': '60px',
                    ...
                    children=html.Div([
                        'Drag and Drop or ',
                        html.A('Select'), ' geojson (.geojson) (not implemented)'
                    ]),
                    style={
                        #'width': '25%',
                        'height': '60px',
        ...
        dcc.Tab(label='Rank Paths', style={'font-weight': 'bold', 'font-size': '120%'}, children=[
            ...
            dcc.Graph(
                            ...
                            )], style = {'margin-left':300,#185,
                                         'margin-right':600,#150,
                                         'margin-top':25,
                                         'textAlign': 'center',
                                         'display':'inline-block'})
        ...
        dcc.Tab(label='Giddy', style={'font-weight': 'bold', 'font-size': '120%'}, children=[
                ...
                html.Div([
                html.P('Select the number of quantile classes (A and a are poorest):', style = {'font-size': '125%', 'margin-top':25, 'font-weight': 'bold'}),
                ...
                html.P('Select the number of spatial lags:', style = {'font-size': '125%', 'margin-top':25, 'font-weight': 'bold'}),
...
def update_markov_pooled_graph(markov_class_value, markov_spatial_value):
    ...
    Heatmap_Layout = dict(title = '<b>Pooled Spatial Markov transition probability matrix</b> <br>Shorrock 1\'s: {}, Shorrock 2\'s: {}, Sommers and Conlisk\'s: {}</br>'.format(round(shorrock_1, 2), round(shorrock_2, 2), round(som_con, 2)),
                          titlefont = {"size": 24,
                                      "family": "Arial"},
                          paper_bgcolor='rgba(0,0,0,0)',
                          plot_bgcolor='rgba(0,0,0,0)')
...
def update_markov_spatial_graph(markov_class_value, markov_spatial_value):
    ...
    fig['layout'].update(title = '<b>Spatial Lags Subplots</b>', 
                         ...
                         paper_bgcolor='rgba(0,0,0,0)',
                         plot_bgcolor='rgba(0,0,0,0)',
                         height = 100 * markov_spatial_value)
    #print(fig['data'])
    #print(fig)
